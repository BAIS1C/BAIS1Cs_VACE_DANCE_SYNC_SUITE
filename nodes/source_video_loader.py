...
class BAIS1C_SourceVideoLoader:
    ...
    def load(self, video):
        ...
        Accepts: string path, or any object with 'video_path' attribute.
        """
        # Accept string or any object with a 'video_path' attribute
        if isinstance(video, str):
            video_path = video
        elif hasattr(video, "video_path"):
            video_path = getattr(video, "video_path")
        else:
            raise ValueError(f"Unsupported video input type: {type(video)}. Expected an object with 'video_path' or a string path.")
        ...
        try:
            ...
            if duration < 30:
                ...
                for test_bpm in loop_bpms:
                    expected_loop_duration = (4 * 60) / test_bpm
