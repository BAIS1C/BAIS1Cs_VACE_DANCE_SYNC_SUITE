# Default/Idle Animation
    ...
    def _get_available_dances(cls) -> List[str]:
        ...
        try:
            # Look for dance_library in current node suite folder
            ...
            dance_names = ["none"]  # Always include none option
    ...
    def __init__(self):
        ...
        # Skeleton connections for rendering (23 main body points)
    ...
    def sync_dance_to_music(self, audio, library_dance, sync_mode, tempo_sync,
        ...
        # 2. LOAD POSE SEQUENCE
        if input_poses is not None:
            # Direct pose tensor input
    ...
    def _analyze_audio(self, audio, fps, debug=False):
        """Analyze audio for music synchronization features"""
        ...
        try:
            ...
            # EQ bands
            bass_mask = (freqs >= 20) & (freqs < 250)     # Bass/low
            mid_mask = (freqs >= 250) & (freqs < 2000)    # Mids  
            high_mask = (freqs >= 2000) & (freqs < 8000)  # Highs
            ...
            # Fallback to amplitude-based analysis
    ...
    def _load_pose_from_library(self, library_dance, debug=False):
        """Load pose sequence from JSON library or create default"""
        if library_dance == "none" or library_dance.startswith("starter_"):
            # Use built-in starter animations
            if debug:
    ...
    def _create_starter_animation(self, animation_name):
        """Create built-in starter animations"""
    ...
    def _create_default_sequence(self):
        """Create a simple default pose sequence (4 frames loop)"""
        ...
        for i in range(4):
            pose = base_pose.copy()
            # Simple arm wave
            pose[7][1] += 0.1 * np.sin(i * np.pi / 2)  # left elbow
            pose[8][1] += 0.1 * np.cos(i * np.pi / 2)  # right elbow
            sequence.append(pose)
        
        return np.array(sequence)

    def _create_hiphop_sequence(self):
        """Create hip-hop style sequence"""
        base_pose = self._create_base_91_pose()
        sequence = []
        ...
        for i in range(8):
            ...
            pose[7][0] += 0.15 * np.sin(t * 4 * np.pi)  # left elbow swing
            pose[8][0] -= 0.15 * np.sin(t * 4 * np.pi)  # right elbow swing
            pose[9][1] += 0.2 * np.abs(np.sin(t * 2 * np.pi))  # left wrist up
            pose[10][1] += 0.2 * np.abs(np.cos(t * 2 * np.pi))  # right wrist up
            
            # Hip sway
            pose[11][0] += 0.08 * np.sin(t * 2 * np.pi)  # left hip
            pose[12][0] -= 0.08 * np.sin(t * 2 * np.pi)  # right hip
    ...
    def _create_ballet_sequence(self):
        ...
        for i in range(6):
            ...
            pose[7][0] -= 0.1  # left elbow out
            pose[8][0] += 0.1  # right elbow out
            pose[7][1] -= 0.05  # slightly raised
            pose[8][1] -= 0.05
            
            pose[9][0] -= 0.15  # wrists further out
            pose[10][0] += 0.15
            pose[9][1] -= 0.1 + 0.05 * np.sin(t * 2 * np.pi)  # gentle wave
            pose[10][1] -= 0.1 + 0.05 * np.cos(t * 2 * np.pi)
            
            # Slight turnout
            pose[15][0] -= 0.02  # left ankle
            pose[16][0] += 0.02  # right ankle
    ...
    def _create_base_91_pose(self):
        """Create base 91-point pose (23 body + 68 face)"""
        # 23 main body points
        body_pose = np.array([
            [0.5, 0.12],   # 0: nose
            [0.5, 0.20],   # 1: neck  
    ...
    def _fit_poses_to_audio(self, pose_sequence, total_frames, sync_mode, debug=False):
        """Fit pose sequence to match audio duration"""
        ...
        if sync_mode == "loop":
            # Loop poses to fill audio duration
            ...
            # Randomly shuffle pose sequence chunks
            fitted = []
            ...
            # Interpolate poses to match audio duration
            if pose_frames == total_frames:
    ...
    def _apply_music_modulation(self, poses, audio_features, eq_assignments, speed_assignments,
        ...
        """Apply music-driven modulation to poses"""
        ...
        for frame_i in range(total_frames):
            ...
            # Apply modulation to specific body parts
            ...
            if frame_signals["head"] > 0.1:
                ...
                for head_idx in range(5):
                    if head_idx < len(pose):
                        pose[head_idx][1] += head_mod * np.sin(frame_i * 0.3)  # Vertical bob
                        pose[head_idx][0] += head_mod * 0.5 * np.cos(frame_i * 0.2)  # Horizontal sway
            ...
            if frame_signals["torso"] > 0.1:
                ...
                # Shoulders
                if 5 < len(pose): pose[5][0] += sway  # left shoulder
                if 6 < len(pose): pose[6][0] -= sway  # right shoulder
                
                # Hips
                if 11 < len(pose): pose[11][0] += sway * 0.7  # left hip
                if 12 < len(pose): pose[12][0] -= sway * 0.7  # right hip
            ...
            if frame_signals["legs"] > 0.1:
                ...
                step_phase = 1 if (frame_i // 4) % 2 == 0 else -1  # Alternating steps
                
                # Knees
                if 13 < len(pose): pose[13][1] += leg_mod * step_phase * 0.5  # left knee
                if 14 < len(pose): pose[14][1] -= leg_mod * step_phase * 0.5  # right knee
                
                # Ankles
                if 15 < len(pose): pose[15][1] += leg_mod * step_phase * 0.3  # left ankle
                if 16 < len(pose): pose[16][1] -= leg_mod * step_phase * 0.3  # right ankle
            ...
            if frame_signals["arms"] > 0.1:
                ...
                # Elbows
                if 7 < len(pose): pose[7][1] += arm_mod * np.sin(frame_i * 0.25)  # left elbow
                if 8 < len(pose): pose[8][1] += arm_mod * np.cos(frame_i * 0.25)  # right elbow
                if 7 < len(pose): pose[7][0] += arm_mod * 0.5 * np.cos(frame_i * 0.2)
                if 8 < len(pose): pose[8][0] -= arm_mod * 0.5 * np.cos(frame_i * 0.2)
                
                # Wrists
                if 9 < len(pose): pose[9][1] += arm_mod * np.cos(frame_i * 0.3 + 0.5)  # left wrist
                if 10 < len(pose): pose[10][1] += arm_mod * np.sin(frame_i * 0.3 + 0.5)  # right wrist
            
            # Ensure pose stays in bounds
    ...
    def _render_pose_sequence(self, pose_sequence, width, height, background_color,
        ...
        """Render pose sequence to video frames"""
        ...
        for frame_idx, pose in enumerate(pose_sequence):
            ...
            # Draw skeleton connections (first 23 points only)
            ...
            # Draw joints
            if show_joints:
                # Main body joints (0-22) - larger circles
                ...
                # Face points (23-90) - smaller circles
    ...
    def _create_stand_arms_dangling(self):
        """Create standing idle pose with arms dangling"""
        return self._create_base_91_pose()  # Default pose is already arms dangling

    def _create_bounce_idle(self):
        """Create subtle bouncing idle animation"""
        base_pose = self._create_base_91_pose()
        sequence = []
        ...
        for i in range(6):
            ...
            # Apply vertical bounce to whole body
            for point_idx in range(23):  # Only body points, not face
    ...
    def _create_hands_on_hips(self):
        """Create hands on hips idle pose"""
        base_pose = self._create_base_91_pose()
        
        # Adjust arm positions for hands on hips
        base_pose[7] = [0.35, 0.40]   # left elbow - out and down
        base_pose[8] = [0.65, 0.40]   # right elbow - out and down
        base_pose[9] = [0.38, 0.62]   # left wrist - at hip level
        base_pose[10] = [0.62, 0.62]  # right wrist - at hip level
...
NODE_DISPLAY_NAME_MAPPINGS = {"BAIS1C_Suite_DancePoser": "🕺 BAIS1C Suite Dance Poser - Music Control Net"}                # Default/Idle Animation
                "default_idle": (["stand_arms_dangling", "bounce", "hands_on_hips"], {"default": "stand_arms_dangling"}),
    ...
    @classmethod
    def _get_available_dances(cls) -> List[str]:
        """Scan dance_library folderimport torch
import numpy as np
import librosa
import json
import os
